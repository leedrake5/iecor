...
def readLangs(lang1, lang2, reverse=False):
    ...
    if reverse:
        pairs = [[normalizeString(l.split('\t')[1]), normalizeString(l.split('\t')[0]), float(l.split('\t')[-4]), float(l.split('\t')[-3]), float(l.split('\t')[-2]), float(l.split('\t')[-1])] for l in lines]
    else:
        pairs = [[normalizeString(l.split('\t')[0]), normalizeString(l.split('\t')[1]), float(l.split('\t')[-4]), float(l.split('\t')[-3]), float(l.split('\t')[-2]), float(l.split('\t')[-1])] for l in lines]
    clade = []
    time = []
    firstnode = []
    proxnode = []
...
#input_lang, output_lang, pairs, input_clade, input_time, input_firstnode, input_proxnode = prepareData('eng', 'fra', True)
...
def newLang(clade, time, firstnode, proxnode):
    new_language = []
    for i in pie_words:
        output_words, _ = evaluate(encoder, decoder, i, input_lang, output_lang, clade, time, firstnode, proxnode)
        output_sentence = ' '.join(output_words)
...
def newLangFrame(clade, time, firstnode, proxnode):
    new_language = newLang(clade, time, firstnode, proxnode)
...
new_lex = newLangFrame(5.0, 8120.0, 5564.0, 800.0)
new_lex.to_csv("~/Downloads/newRomance_2.csv")
...
checkpoint = torch.load("~GitHub/iecor/pyTorch Runs/second_best_model.pth")
