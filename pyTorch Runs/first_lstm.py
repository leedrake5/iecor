...
def readLangs(lang1, lang2, reverse=False):
    ...
    pairs = [[normalizeString(l.split('\t')[0]), normalizeString(l.split('\t')[1]), float(l.split('\t')[-2]), float(l.split('\t')[-1])] for l in lines]
    clade = []
    time = []
    ...
    if reverse:
        ...
        for pair in pairs :
            clade.append(pair[2])
            time.append(pair[3])
    return input_lang, output_lang, pairs, clade, time
...
def prepareData(lang1, lang2, reverse=False):
    input_lang, output_lang, pairs, input_clade, input_time = readLangs(lang1, lang2, reverse)
    ...
    return input_lang, output_lang, pairs, input_clade, input_time

#input_lang, output_lang, pairs, input_clade, input_time = prepareData('eng', 'fra', True)
...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_ids, input_clade, input_time):
        ...
        output = torch.cat((output, input_clade_processed, input_time_processed), dim=1)   # Change this line from dim=1 to dim=2
...
def get_dataloader(batch_size):
    input_lang, output_lang, pairs, clade, time = prepareData('eng', 'fra', False)  # Modify the return values here
    n = len(pairs)
...
def train_epoch(dataloader, encoder, decoder, encoder_optimizer,
    ...
    for data in dataloader:
        input_ids, target_tensor, input_clade, input_time = data
        encoder_optimizer.zero_grad()
        decoder_optimizer.zero_grad()
        encoder_outputs, encoder_hidden = encoder(input_ids, input_clade, input_time)  # Pass the new inputs to EncoderRNN
...
def train(train_dataloader, encoder, decoder, n_epochs, learning_rate=0.001, print_every=100, plot_every=100):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if loss < best_loss:
            best_loss = loss
            torch.save({'encoder': encoder.state_dict(), 'decoder': decoder.state_dict(), 'loss': best_loss}, "~/GitHub/iecor/pyTorch Runs/best_model.pth")
    showPlot(plot_losses, "~/GitHub/iecor/pyTorch Runs/progress_1.png")
...
def evaluate(encoder, decoder, sentence, input_lang, output_lang, input_clade, input_time):
    with torch.no_grad():
        ...
        encoder_outputs, encoder_hidden = encoder(input_tensor, input_clade, input_time)
...
def evaluate_all(encoder, decoder, sentence, input_lang, output_lang, train_dataloader):
    with torch.no_grad():
        input_tensor = tensorFromSentence(input_lang, sentence)
        encoder_outputs, encoder_hidden = encoder(input_tensor, tensor(input_clade), tensor(input_time))
        decoder_outputs, decoder_hidden, decoder_attn = decoder(encoder_outputs, encoder_hidden)
        _, topi = decoder_outputs.topk(1)
        decoded_ids = topi.squeeze()
        decoded_words = []
...
def evaluateRandomly(encoder, decoder, n=10):
    for i in range(n):
        ...
        input_clade = pair[2]
        input_time = pair[3]
        output_words, _ = evaluate(encoder, decoder, str(pair[0]), input_lang, output_lang, input_clade, input_time)
        output_sentence = ' '.join(output_words)
        print('<', output_sentence)
        print('')
        
def newLang(clade, time):
    new_language = []
    for i in pie_words:
        output_words, _ = evaluate(encoder, decoder, i, input_lang, output_lang, clade, time)
        output_sentence = ' '.join(output_words)
